...
class MyData extends DataTableSource {
  ...
  DataRow getRow(int index) {
    return DataRow(cells: [
      ...
      DataCell(Text(listPendapatan[index]['NOTRANSAKSI'],
          style:
              TextStyle(fontWeight: FontWeight.bold, color: Colors.grey[800]))),
      DataCell(Text(listPendapatan[index]['STATUSPOSTING'],
          style:
              TextStyle(fontWeight: FontWeight.bold, color: Colors.grey[800]))),
      DataCell(Center(
        child: Row(
          ...
          children: [
            ButtonEdit(satuan: listPendapatan[index]['NOTRANSAKSI']),
          ],
        ),
      )),
    ]);
  ...
  @override
  int get rowCount => listPendapatan.length;

  @override
  int get selectedRowCount => 0;
}

class TablePendapatan extends StatefulWidget {
  List<Map<String, dynamic>> listPendapatan;
  TablePendapatan({Key key, @required this.listPendapatan}) : super(key: key);

  @override
  State<TablePendapatan> createState() => _TablePendapatanState();
}

class _TablePendapatanState extends State<TablePendapatan> {
  @override
  Widget build(BuildContext context) {
    final screenWidth = MediaQuery.of(context).size.width;
    final screenHeight = MediaQuery.of(context).size.height;
    final DataTableSource myTable = MyData(widget.listPendapatan);
